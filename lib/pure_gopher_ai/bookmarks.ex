defmodule PureGopherAi.Bookmarks do
  ...
  @doc """
  Adds a bookmark for a user. Requires passphrase authentication.
  """
  def add(username, passphrase, selector, title, folder \\ "default") do
    GenServer.call(__MODULE__, {:add, username, passphrase, selector, title, folder})
  end

  @doc """
  Removes a bookmark. Requires passphrase authentication.
  """
  def remove(username, passphrase, bookmark_id) do
    GenServer.call(__MODULE__, {:remove, username, passphrase, bookmark_id})
  end

  @doc """
  Gets all bookmarks for a user. Requires passphrase authentication.
  """
  def list(username, passphrase, folder \\ nil) do
    GenServer.call(__MODULE__, {:list, username, passphrase, folder})
  end

  @doc """
  Gets all folders for a user. Requires passphrase authentication.
  """
  def folders(username, passphrase) do
    GenServer.call(__MODULE__, {:folders, username, passphrase})
  end

  @doc """
  Creates a new folder. Requires passphrase authentication.
  """
  def create_folder(username, passphrase, folder_name) do
    GenServer.call(__MODULE__, {:create_folder, username, passphrase, folder_name})
  end

  @doc """
  Deletes a folder (and moves bookmarks to default). Requires passphrase authentication.
  """
  def delete_folder(username, passphrase, folder_name) do
    GenServer.call(__MODULE__, {:delete_folder, username, passphrase, folder_name})
  end

  @doc """
  Moves a bookmark to a different folder. Requires passphrase authentication.
  """
  def move(username, passphrase, bookmark_id, new_folder) do
    GenServer.call(__MODULE__, {:move, username, passphrase, bookmark_id, new_folder})
  end

  @doc """
  Gets a single bookmark by ID. Requires passphrase authentication.
  """
  def get(username, passphrase, bookmark_id) do
    GenServer.call(__MODULE__, {:get, username, passphrase, bookmark_id})
  end

  @doc """
  Gets bookmark count for a user. Requires passphrase authentication.
  """
  def count(username, passphrase) do
    GenServer.call(__MODULE__, {:count, username, passphrase})
  end

  @doc """
  Exports bookmarks as a simple text format. Requires passphrase authentication.
  """
  def export(username, passphrase) do
    GenServer.call(__MODULE__, {:export, username, passphrase})
  end
  ...
  @impl true
  def handle_call({:add, username, passphrase, selector, title, folder}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
        folder = sanitize_folder(folder)
        title = sanitize_title(title)

        # Check bookmark limit
        current_count = get_bookmark_count(username_lower)

        if current_count >= @max_bookmarks_per_user do
          {:reply, {:error, :limit_reached}, state}
        else
          # Check if already bookmarked
          existing = get_user_bookmarks(username_lower)
          ...
          if already_exists do
            ...
            user_data = get_user_data(username_lower)
            ...
            :dets.insert(@table_name, {username_lower, updated_data})
            :dets.sync(@table_name)

            Logger.info("[Bookmarks] #{username_lower} added bookmark: #{selector}")
  ...
  @impl true
  def handle_call({:remove, username, passphrase, bookmark_id}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
        ...
        if bookmark do
          ...
          Logger.info("[Bookmarks] #{username_lower} removed bookmark: #{bookmark.selector}")
          {:reply, :ok, state}
        else
  ...
  @impl true
  def handle_call({:list, username, passphrase, folder}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
        bookmarks = get_user_bookmarks(username_lower)
  ...
  @impl true
  def handle_call({:folders, username, passphrase}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
        user_data = get_user_data(username_lower)
  ...
  @impl true
  def handle_call({:create_folder, username, passphrase, folder_name}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
  ...
  @impl true
  def handle_call({:delete_folder, username, passphrase, folder_name}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
  ...
  @impl true
  def handle_call({:move, username, passphrase, bookmark_id, new_folder}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
  ...
  @impl true
  def handle_call({:get, username, passphrase, bookmark_id}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
        bookmarks = get_user_bookmarks(username_lower)
  ...
  @impl true
  def handle_call({:count, username, passphrase}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
        count = get_bookmark_count(username_lower)
  ...
  @impl true
  def handle_call({:export, username, passphrase}, _from, state) do
    case UserProfiles.authenticate(username, passphrase) do
      {:ok, _profile} ->
        username_lower = sanitize_username(username)
        bookmarks = get_user_bookmarks(username_lower)
